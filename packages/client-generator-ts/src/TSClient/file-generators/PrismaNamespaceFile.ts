...
function buildClientOptions(context: GenerateContext) {
  ...
  // TypeScript reports missing-property errors for a discriminated union
  // against the later-declared branch, so the adapter branch must come last to
  // make `new PrismaClient({ log: [...] })` report "adapter is missing" (the
  // recommended option for most users) rather than "accelerateUrl is missing".
  // This relies on `prismaNamespace.ts` being emitted *without* `// @ts-nocheck`
  // (see `utils/addPreamble.ts`); with `@ts-nocheck` the declaration-order
  // heuristic doesn't apply and TypeScript picks the alphabetically-first
  // branch instead.
